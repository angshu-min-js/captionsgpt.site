...
function populateContent(config) {
  ...
  //  imgElement.src = config.image.src;
   ...
   // videoElement.src = config.image.src;
    ...
    youtubeEmbed.src = `https://www.youtube.com/embed/${config.image.src}?autoplay=1&loop=1&playlist=${config.image.src}&controls=0&modestbranding=1`;
    ...
    footerLove.innerHTML = `&copy;${config.footer.year} All rights reserved &mdash; <a href="https://qloudai.app/" target="_blank">${config.footer.company_name}</a> | Reach Us - <a href="mailto:${config.footer.support_email}" target="_blank">${config.footer.support_email_display}</a> |&nbsp`;
